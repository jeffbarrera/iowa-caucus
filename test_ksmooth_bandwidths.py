...
from jf_utils import get_file_as_named_rows, candidates, results, convert_nonints_to_na, estimate_epa, estimate_ksmooth
        ...
        for scope in ['iowa']:
            ...
            for year in [2008, 2012]:
                ...
                for candidate in candidates[year]:
                    ...
                    est_value = estimate_ksmooth(bandwidth)
                    squared_error = (candidate_result - est_value)*(candidate_result - est_value)
                    row_result = {'window_type':'ksmooth', 'bw_value':bandwidth, 'year':year, 'scope':scope, 'candidate':candidate, 'result':candidate_result, 'estimate':est_value, 'squared_error':squared_error}
                    print row_result
            ...
            mse = se/count
            mse_result = {'window_type':'ksmooth', 'bw_value':bandwidth, 'scope':scope, 'mse':mse, 'se':se}
