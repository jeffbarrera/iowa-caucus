...
from jf_utils import get_file_as_named_rows, candidates, results, convert_nonints_to_na, estimate_epa, estimate_ksmooth, estimate_lowess
...
if __name__ == "__main__":
    ...
    mse_outfilename  = "lowesslog_mse.csv"
    ...
    for bandwidth in [0.5,0.1,0.15,0.2,0.25,0.3,0.35,0.4]:
        ...
        for scope in ['iowa']:
            ...
            for year in [2008, 2012]:
                ...
                for candidate in candidates[year]:
                    ...
                    # apparently the robjects namespace works from there
                    #est_value = estimate_epa(bandwidth)
                    est_value = estimate_lowess(bandwidth)
                    squared_error = (candidate_result - est_value)*(candidate_result - est_value)
                    row_result = {'window_type':'lowess', 'bw_value':bandwidth, 'year':year, 'scope':scope, 'candidate':candidate, 'result':candidate_result, 'estimate':est_value, 'squared_error':squared_error}
                    print row_result
            ...
            mse = se/count
            mse_result = {'window_type':'lowess', 'bw_value':bandwidth, 'scope':scope, 'mse':mse, 'se':se}
